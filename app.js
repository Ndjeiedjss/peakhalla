...
const translations = {
  en: {
    ...
    firstSeen: 'First seen', lastSeen: 'Last seen', formerly: 'Formerly', aliasMatch: 'Old-name match', main: 'Main', unknownLegend: 'Main unknown', portraitLoading: 'Loading main legend…', recordSyncing: 'Record not reported by the source', prizePool: 'Prize pool', tournamentUpcoming: 'Upcoming', tournamentLive: 'Live now', tournamentCompleted: 'Completed', tournamentAnnounced: 'Announced',
  ...
  ar: {
    ...
    firstSeen: 'أول ظهور', lastSeen: 'آخر ظهور', formerly: 'كان اسمه', aliasMatch: 'تطابق اسم قديم', main: 'الأساسية', unknownLegend: 'الشخصية غير معروفة', portraitLoading: 'نحمّل الشخصية الأساسية…', recordSyncing: 'السجل غير متوفر من المصدر', prizePool: 'مجموع الجوائز', tournamentUpcoming: 'قادمة', tournamentLive: 'مباشرة الآن', tournamentCompleted: 'انتهت', tournamentAnnounced: 'معلنة',
...
function applyLanguage() {
  ...
  if (isLiveQueuePage) {
    ...
    if (state.tournamentData && esportsRouteView === 'tournaments') renderTournamentDirectory(state.tournamentData);
...
function legendImageMarkup(legend, options = {}) {
  ...
  if (!candidates.length) return `<span class="legend-image-shell is-fallback" data-fallback="${fallback}"><span class="legend-fallback">${fallback}</span></span>`;
...
function wait(ms) { return new Promise((resolve) => window.setTimeout(resolve, Math.max(0, Number(ms) || 0))); }

async function getJson(url, options = {}) {
  const attempts = Math.max(1, 1 + Math.min(2, Number(options.retries || 0)));
  let lastError = null;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        const error = new Error(data.error || t('fetchError'));
      ...
      const transient = lastError?.name === 'TimeoutError'
        || lastError?.name === 'TypeError'
        || [408, 425, 429, 500, 502, 503, 504].includes(Number(lastError?.status));
      if (!transient || attempt >= attempts - 1) throw lastError;
      const baseDelay = Number(options.retryDelayMs || 500) * (attempt + 1);
      await wait(Math.max(baseDelay, Number(lastError?.retryAfterMs || 0)));
    } finally {
      window.clearTimeout(timer);
      options.signal?.removeEventListener('abort', abortFromCaller);
    }
  }
  throw lastError || new Error(t('fetchError'));
}
...
function playerPayloadSignature(data = {}) {
  ...
  const regionRanks = (player.region_ranks || []).map((item) => [item.region, item.rank]).sort((a, b) => String(a[0]).localeCompare(String(b[0])));
...
const PLAYER_BROWSER_CACHE_KEY = 'peakhalla-player-cache-v7';
const PLAYER_BROWSER_CACHE_MAX_AGE_MS = 24 * 60 * 60_000;

function profileCompletenessScore(data = {}) {
  const player = data?.player || {};
  let score = 0;
  if (player.brawlhalla_id) score += 5;
  if (player.name && !/^Player\s+\d+$/i.test(player.name)) score += 12;
  if (Number.isFinite(Number(player.rating))) score += 12;
  if (Number.isFinite(Number(player.peak_rating))) score += 6;
  if (player.tier && player.tier !== 'Unranked') score += 4;
  if (Number.isFinite(Number(player.account_xp)) && Number(player.account_xp) > 0) score += 10;
  if (Number.isFinite(Number(player.level)) && Number(player.level) > 0) score += 6;
  if (player.main_legend) score += 8;
  if ((player.lifetime_legends || []).length) score += 8;
  if ((player.legends || []).length) score += 6;
  if ((data.known_names || []).length) score += 5;
  if (player.data_quality?.official_ranked_ok) score += 8;
  if (player.data_quality?.official_lifetime_ok || player.data_quality?.corehalla_enriched) score += 8;
  if (player.data_quality?.official_guild_ok) score += 2;
  return score;
}

function readPlayerBrowserCache(id) {
  try {
    const rawCache = localStorage.getItem(PLAYER_BROWSER_CACHE_KEY) || localStorage.getItem('peakhalla-player-cache-v6') || localStorage.getItem('peakhalla-player-cache-v5') || '{}';
    const all = JSON.parse(rawCache);
    ...
    if (data?.player) {
      data.player.data_quality = { ...(data.player.data_quality || {}), browser_snapshot: true };
    }
    return data;
  } catch { return null; }
}

function readPlayerBootstrap(id) {
  const node = document.getElementById('peakhalla-player-bootstrap');
  if (!node || node.dataset.consumed === '1') return null;
  try {
    const data = JSON.parse(node.textContent || 'null');
    if (!data?.player || Number(data.player.brawlhalla_id) !== Number(id)) return null;
    node.dataset.consumed = '1';
    writePlayerBrowserCache(id, data);
    return data;
  } catch {
    node.dataset.consumed = '1';
    return null;
...
function writePlayerBrowserCache(id, data) {
  ...
  try {
    ...
    const key = String(id);
    const incomingScore = profileCompletenessScore(data);
    const existing = all[key];
    const existingScore = Number(existing?.score ?? profileCompletenessScore(existing?.data || {}));
    const existingAge = Date.now() - Number(existing?.saved_at || 0);
    if (existing?.data && existingAge < PLAYER_BROWSER_CACHE_MAX_AGE_MS && incomingScore + 4 < existingScore) return;
    all[key] = { saved_at: Date.now(), score: incomingScore, data };
...
function prefetchPlayerProfile(id) {
  ...
  const promise = fetch(`/api/player/${encodeURIComponent(numericId)}?fast=1&prefetch=1&_=${Date.now()}`, {
    ...
    .then((data) => {
      if (data?.player) {
        writePlayerBrowserCache(numericId, data);
        state.playerSeeds.set(numericId, data);
      }
      return data;
    })
    .catch(() => null)
...
function setupPlayerPrefetch(container, selector, datasetKey) {
  ...
  for (const button of buttons) {
    ...
    button.addEventListener('pointerdown', () => warm(button), { once: true, passive: true });
...
function enrichRenderedPortraits(container, buttonSelector, portraitSelector, concurrency = 3) {
  if (!container) return;
  const enrichmentKey = `${buttonSelector}|${portraitSelector}`;
  const activeKey = container.dataset.portraitEnrichmentKey;
  if (activeKey === enrichmentKey && container.dataset.portraitEnriching === '1') return;
  ...
  const attempt = async (attemptIndex = 0) => {
    ...
    if (!targets.length) {
      delete container.dataset.portraitEnriching;
      return;
    }
    ...
    stabilizeLegendImages(container);

    const remaining = [...container.querySelectorAll(buttonSelector)]
      .filter((button) => button.dataset.needsPortrait === '1' && Number(button.dataset.playerId) > 0);
    if (!remaining.length || attemptIndex >= retryDelays.length - 1) {
      delete container.dataset.portraitEnriching;
      return;
    }
    window.setTimeout(() => attempt(attemptIndex + 1).catch(() => null), retryDelays[attemptIndex + 1]);
  };

  attempt(0).catch(() => { delete container.dataset.portraitEnriching; });
}
...
function navigateToPlayer(id, seed = null) {
  ...
  const preparedSeed = seed || state.playerSeeds.get(playerId);
  ...
  const go = (data = null) => {
    if (navigated) return;
    navigated = true;
    if (data?.player) writePlayerNavigationSeed(data);
    location.href = `/player/${encodeURIComponent(playerId)}`;
  };
  const fallbackTimer = window.setTimeout(() => go(preparedSeed), 780);
  prefetchPlayerProfile(playerId).then((data) => {
    if (navigated) return;
    window.clearTimeout(fallbackTimer);
    go(data || preparedSeed);
  }).catch(() => null);
}
...
function patchRenderedPlayerRankedSummary(merged) {
  if (!merged?.player || currentRenderedPlayerId() !== Number(merged.player.brawlhalla_id)) return;
  const player = merged.player;
  state.currentPlayer = merged;
  const currentElo = Number(player.rating);
  const peakElo = Number(player.peak_rating);
  const hasCurrent = Number.isFinite(currentElo);
  const hasPeak = Number.isFinite(peakElo);
  const regionalRank = player.region_ranks?.find((rank) => rank.region === player.region)?.rank;
  const displayNumber = (value) => value === null || value === undefined || value === '' || !Number.isFinite(Number(value)) ? '—' : number(value);
  const setText = (selector, value) => { const node = $(selector); if (node) node.textContent = value; };
  setText('#player-title', player.name || `Player ${player.brawlhalla_id}`);
  setText('#player-region', player.region || '—');
  setText('#player-tier', player.tier || 'Unranked');
  setText('#player-rating', hasCurrent ? number(currentElo) : '—');
  setText('#updated-at', dateTime(player.updated_at));
  if (els.playerPeakRating) els.playerPeakRating.textContent = hasPeak ? number(peakElo) : '—';
  const gap = hasCurrent && hasPeak ? Math.max(0, peakElo - currentElo) : null;
  if (els.eloGap) els.eloGap.textContent = gap === null ? '—' : (gap === 0 ? t('atPeak') : `${number(gap)} ${t('offPeak')}`);
  if (els.rankProgressFill) {
    const progress = hasCurrent && hasPeak && peakElo > 0 ? Math.max(8, Math.min(100, (currentElo / peakElo) * 100)) : 0;
    els.rankProgressFill.style.width = `${progress}%`;
  }
  const statsGrid = $('#stats-grid');
  if (statsGrid) statsGrid.innerHTML = [
    statCard(t('globalRank'), player.global_rank ? `#${number(player.global_rank)}` : '—', 'WORLD POSITION'),
    statCard(t('rankedGames'), displayNumber(player.ranked_games), 'CURRENT SEASON'),
    statCard(t('rankedWins'), displayNumber(player.ranked_wins), 'CURRENT SEASON'),
    statCard(t('winRate'), player.ranked_win_rate === null || player.ranked_win_rate === undefined ? '—' : `${number(player.ranked_win_rate)}%`, 'CURRENT SEASON')
  ].join('');
  const summary = $('#player-summary');
  if (summary) summary.innerHTML = `<div><dt>${escapeHtml(t('accountLevel'))}</dt><dd>${displayNumber(player.level)}</dd></div><div><dt>${escapeHtml(t('regionRank'))}</dt><dd>${regionalRank ? `#${number(regionalRank)}` : '—'}</dd></div><div><dt>${escapeHtml(t('totalGames'))}</dt><dd>${displayNumber(player.lifetime_games)}</dd></div><div><dt>${escapeHtml(t('totalWins'))}</dt><dd>${displayNumber(player.lifetime_wins)}</dd></div><div><dt>${escapeHtml(t('overallWinRate'))}</dt><dd>${player.lifetime_win_rate === null || player.lifetime_win_rate === undefined ? '—' : `${number(player.lifetime_win_rate)}%`}</dd></div>`;
  if (els.profile) els.profile.dataset.liveState = playerResponseIsPartial(merged) ? 'refreshing' : 'current';
  if (els.statsSourceNote) {
    const label = els.statsSourceNote.querySelector('span');
    if (label) label.textContent = t('liveOfficialData');
  }
  state.playerSignature = playerPayloadSignature(merged);
  if (!playerResponseIsPartial(merged)) writePlayerBrowserCache(player.brawlhalla_id, merged);
}
...
async function refreshPlayerRankedLive(id) {
  ...
  if (state.playerRankedController) return state.currentPlayer;
  const controller = new AbortController();
  state.playerRankedController = controller;
  try {
    const data = await getJson(`/api/player/${encodeURIComponent(playerId)}/ranked-live?_=${Date.now()}`, {
      signal: controller.signal,
      timeoutMs: 12000,
      retries: 1,
      retryDelayMs: 450
    ...
    if (data.no_update || (data.api_unavailable && data.player.rating === undefined)) return state.currentPlayer;
    ...
    merged.player = {
      ...
      ...data.player,
      data_quality: {
        ...(merged.player?.data_quality || {}),
        official_ranked_ok: data.stale ? Boolean(merged.player?.data_quality?.official_ranked_ok) : true,
        ranked_live_checked: true,
        ranked_live_stale: Boolean(data.stale),
        fetched_at: data.updated_at || new Date().toISOString()
      }
    };
    merged.partial = Boolean(merged.partial && !merged.player.data_quality?.official_lifetime_ok);
    patchRenderedPlayerRankedSummary(merged);
    if (!data.stale) state.playerLastRankedRefreshAt = Date.now();
    return merged;
  } catch (error) {
    return state.currentPlayer;
  } finally {
    if (state.playerRankedController === controller) state.playerRankedController = null;
  }
}
...
async function refreshPlayerAliasesLive(id) {
  ...
  try {
    const data = await getJson(`/api/player/${encodeURIComponent(playerId)}/aliases?refresh=1&_=${Date.now()}`, { signal: controller.signal, timeoutMs: 12000, retries: 1, retryDelayMs: 500 });
    ...
    state.playerLastAliasRefreshAt = Date.now();
    return names;
  } catch (error) {
...
async function refreshPlayerGuildLive(id) {
  const playerId = Number(id);
  if (!Number.isSafeInteger(playerId) || playerId <= 0 || currentRenderedPlayerId() !== playerId) return null;
  state.playerGuildController?.abort();
  const controller = new AbortController();
  state.playerGuildController = controller;
  try {
    const data = await getJson(`/api/player/${encodeURIComponent(playerId)}/guild-live?_=${Date.now()}`, {
      signal: controller.signal,
      timeoutMs: 9000,
      retries: 0
    });
    if (controller.signal.aborted || currentRenderedPlayerId() !== playerId || !data?.verified) return null;
    if (state.currentPlayer?.player) {
      state.currentPlayer.player.guild = data.guild || null;
      state.currentPlayer.player.data_quality = {
        ...(state.currentPlayer.player.data_quality || {}),
        official_guild_ok: true,
        guild_roster_checked: Boolean(data.roster_checked),
        guild_checked_at: data.updated_at || new Date().toISOString()
      };
      renderPlayerClan(data.guild || null);
      writePlayerBrowserCache(playerId, state.currentPlayer);
      state.playerSignature = playerPayloadSignature(state.currentPlayer);
    }
    state.playerLastGuildRefreshAt = Date.now();
    return data.guild || null;
  } catch (error) {
    return null;
  } finally {
    if (state.playerGuildController === controller) state.playerGuildController = null;
  }
}

function schedulePlayerLiveRefresh(id, delayMs = 700) {
  window.clearTimeout(state.playerAutoRefreshTimer);
  state.playerAutoRefreshTimer = window.setTimeout(() => {
    const activePlayerId = currentRenderedPlayerId() || Number(standalonePlayerId || 0);
    if (document.visibilityState !== 'visible' || activePlayerId !== Number(id)) return;
    loadPlayer(id, { background: true, forceFresh: false, shouldScroll: false, silent: true, autoVerify: false }).catch(() => null);
  }, Math.max(250, Number(delayMs) || 700));
}

function schedulePlayerLivePoll(id) {
  window.clearTimeout(state.playerLivePollTimer);
  state.playerLivePollTimer = window.setTimeout(async () => {
    const activePlayerId = currentRenderedPlayerId() || Number(standalonePlayerId || 0);
    if (document.visibilityState !== 'visible' || activePlayerId !== Number(id)) return;
    await refreshPlayerRankedLive(id).catch(() => null);
    if (Date.now() - Number(state.playerLastGuildRefreshAt || 0) > 3 * 60_000) refreshPlayerGuildLive(id).catch(() => null);
    if (Date.now() - Number(state.playerLastAliasRefreshAt || 0) > 10 * 60_000) refreshPlayerAliasesLive(id).catch(() => null);
    schedulePlayerLivePoll(id);
  }, 2 * 60_000);
}
...
async function loadPlayer(id, options = {}) {
  ...
  try {
    if (!silent && !manualRefresh && !backgroundRefresh) {
      ...
      window.clearTimeout(state.playerLivePollTimer);
      const candidates = [
        readPlayerBootstrap(playerId),
        readPlayerBrowserCache(playerId),
        readPlayerNavigationSeed(playerId)
      ].filter(Boolean);
      const cached = candidates.sort((left, right) => profileCompletenessScore(right) - profileCompletenessScore(left))[0] || null;
      if (cached) {
    ...
    const profileAlreadyVisible = currentRenderedPlayerId() === playerId && Boolean(state.currentPlayer?.player);
    if (!silent && !renderedInstantData && !(manualRefresh && profileAlreadyVisible)) showStatus(manualRefresh ? t('refreshingLiveStats') : t('loadingPlayer'));

    const requestMode = manualRefresh ? 'refresh' : (backgroundRefresh ? 'live' : 'fast');
    const params = new URLSearchParams({ [requestMode]: '1', _: String(Date.now()) });
    if (manualRefresh) {
      state.playerRefreshController?.abort();
      state.playerRefreshController = new AbortController();
    }

    const timeoutMs = manualRefresh ? 38000 : (backgroundRefresh ? 26000 : 9000);
    ...
    const hasCurrentRenderedProfile = currentRenderedPlayerId() === playerId && Boolean(state.currentPlayer?.player);
    const shouldRender = !hasCurrentRenderedProfile || changed;
    if (shouldRender) {
      renderPlayer(data, renderedInstantData ? false : options.shouldScroll !== false, {
        refreshSecondary: !backgroundRefresh && (!renderedInstantData || renderedSeedOnly)
      });
    ...
    if (!backgroundRefresh && !manualRefresh) {
      // The complete saved profile is already visible. Only patch the fields
      // that can change quickly; do not rebuild the whole page after opening.
      window.setTimeout(() => refreshPlayerRankedLive(playerId).catch(() => null), 80);
      window.setTimeout(() => refreshPlayerGuildLive(playerId).catch(() => null), 180);
      window.setTimeout(() => refreshPlayerAliasesLive(playerId).catch(() => null), 500);
      if (partial) schedulePlayerLiveRefresh(playerId, Math.max(2500, Number(data?.retry_after_ms || 2500)));
      else schedulePlayerLivePoll(playerId);
    } else if (backgroundRefresh) {
      state.playerLastVerifiedAt = Date.now();
      if (partial && state.playerRetryCount < 2) {
        state.playerRetryCount += 1;
        schedulePlayerLiveRefresh(playerId, state.playerRetryCount === 1 ? 12000 : 30000);
      } else {
        state.playerRetryCount = 0;
    ...
    const hasVisibleProfile = currentRenderedPlayerId() === playerId && Boolean(state.currentPlayer?.player);
    if (hasVisibleProfile) {
      hideStatus();
      if (state.playerRetryCount < 2) {
        state.playerRetryCount += 1;
        schedulePlayerLiveRefresh(playerId, state.playerRetryCount === 1 ? 5000 : 20000);
      }
      schedulePlayerLivePoll(playerId);
      return state.currentPlayer;
    }
    state.playerRetryCount += 1;
    showStatus(t('loadingPlayer'));
    if (state.playerRetryCount <= 3) schedulePlayerLiveRefresh(playerId, [1200, 4000, 12000][state.playerRetryCount - 1]);
    return null;
  } finally {
...
function renderLeaderboardRows(rankings = [], options = {}) {
  ...
  warmVisiblePlayerProfiles(els.leaderboard, 10);
  activateImageFallbacks();
...
const LEADERBOARD_BROWSER_CACHE_VERSION = 'v5-official-only-page50';
...
function readLeaderboardBrowserCache(region, mode, page = 1) {
  try {
    const cached = JSON.parse(localStorage.getItem(leaderboardBrowserCacheKey(region, mode, page)) || 'null');
    if (!cached?.rankings || cached.source !== 'official' || Date.now() - Number(cached.savedAt || 0) > LEADERBOARD_BROWSER_CACHE_MAX_AGE_MS) return null;
    return cached;
  } catch {
    return null;
  }
...
function writeLeaderboardBrowserCache(region, mode, page, data) {
  if (data?.source !== 'official' || !Array.isArray(data.rankings) || !data.rankings.length) return;
  try {
    localStorage.setItem(leaderboardBrowserCacheKey(region, mode, page), JSON.stringify({
      savedAt: Date.now(),
...
function tournamentCard(event, index) {
  ...
  const status = ['live', 'upcoming', 'completed', 'announced'].includes(String(event.status || '').toLowerCase())
    ? String(event.status).toLowerCase()
    : 'announced';
  const statusLabel = {
    live: t('tournamentLive'),
    upcoming: t('tournamentUpcoming'),
    completed: t('tournamentCompleted'),
    announced: t('tournamentAnnounced')
  }[status];
  ...
  return `${linkStart}<div class="tournament-card-art${posterClass}" data-region="${escapeHtml(primaryRegion)}"${posterStyle}><span>BRAWLHALLA</span><strong>${escapeHtml((event.series || event.name || 'TOURNAMENT').replace(/\s+20\d{2}.*/i, '').slice(0, 28))}</strong><b>${escapeHtml(primaryMode)}</b><small>${escapeHtml(primaryRegion)}</small></div><div class="tournament-card-copy"><div class="tournament-card-badges"><b class="event-region">${escapeHtml(tournamentRegionName(primaryRegion))}</b><em class="tournament-status ${escapeHtml(status)}">${escapeHtml(statusLabel)}</em>${modes.map((mode) => `<em class="tournament-mode-badge">${escapeHtml(mode)}</em>`).join('')}<em>${escapeHtml(category)}</em></div><h4>${escapeHtml(event.name || 'Brawlhalla Tournament')}</h4><p>${escapeHtml(event.note || event.description || '')}</p>${prize ? `<div class="tournament-prize"><span>${escapeHtml(t('prizePool'))}</span><strong>${escapeHtml(prize)}</strong></div>` : ''}<div class="tournament-card-meta">${date ? `<span>${escapeHtml(status === 'completed' ? t('tournamentCompleted') : t('tournamentUpcoming'))}: ${escapeHtml(date)}</span>` : ''}${sourceHost ? `<span>${escapeHtml(sourceHost)}</span>` : ''}</div></div><span class="tournament-card-arrow">↗</span>${linkEnd}`;
}

function tournamentDirectoryCacheKey(type, region, mode) {
  return `peakhalla-tournaments-v4:${type}:${region}:${mode}`;
}

function readTournamentDirectoryCache(type, region, mode) {
  try {
    const entry = JSON.parse(localStorage.getItem(tournamentDirectoryCacheKey(type, region, mode)) || 'null');
    if (!entry?.data?.events || Date.now() - Number(entry.savedAt || 0) > 30 * 60_000) return null;
    return entry.data;
  } catch { return null; }
}

function writeTournamentDirectoryCache(type, region, mode, data) {
  try { localStorage.setItem(tournamentDirectoryCacheKey(type, region, mode), JSON.stringify({ savedAt: Date.now(), data })); }
  catch {}
}

function tournamentModeSection(mode, events = []) {
  if (!events.length) return '';
  const heading = mode === '1v1' ? t('tournamentSingles') : mode === '2v2' ? t('tournamentDoubles') : t('tournamentOtherModes');
...
function tournamentDirectorySignature(data = {}) {
  return JSON.stringify((data.events || []).map((event) => [event.id, event.name, event.date, event.status, event.prize_pool, event.image_url, event.source_url]));
}
...
function renderTournamentDirectory(data = {}) {
  ...
  if (!events.length) {
    els.tournamentDirectoryList.innerHTML = `<p class="empty-copy tournament-directory-empty">${escapeHtml(t('noTournaments'))}</p>`;
    return;
  }

  const selectedMode = state.tournamentMode || els.tournamentDirectoryMode?.value || 'ALL';
  if (selectedMode === 'ALL') {
    const singles = events.filter((event) => Array.isArray(event.modes) && event.modes.includes('1v1') && !event.modes.includes('2v2'));
    const doubles = events.filter((event) => Array.isArray(event.modes) && event.modes.includes('2v2') && !event.modes.includes('1v1'));
    const other = events.filter((event) => !singles.includes(event) && !doubles.includes(event));
    els.tournamentDirectoryList.innerHTML = [
      tournamentModeSection('1v1', singles),
      tournamentModeSection('2v2', doubles),
      tournamentModeSection('other', other)
    ].join('');
  } else {
    els.tournamentDirectoryList.innerHTML = tournamentModeSection(selectedMode, events);
  }
}
...
async function loadTournamentDirectory(options = {}) {
  ...
  const type = state.tournamentType || 'official';
  const params = new URLSearchParams({ type, region, mode });
  syncEsportsPageUrl();
  if (options.refresh) params.set('refresh', '1');

  const cached = !options.refresh ? readTournamentDirectoryCache(type, region, mode) : null;
  if (cached) renderTournamentDirectory(cached);
  else if (!options.silent) els.tournamentDirectoryList.innerHTML = `<div class="tournament-directory-loading"><span></span><strong>${escapeHtml(t('loadingTournaments'))}</strong></div>`;
  if (els.tournamentDirectoryRefresh) els.tournamentDirectoryRefresh.disabled = true;
  try {
    const data = await getJson(`/api/esports/tournaments?${params}`, { timeoutMs: 9500 });
    renderTournamentDirectory(data);
    writeTournamentDirectoryCache(type, region, mode, data);
  } catch (error) {
    if (!cached) els.tournamentDirectoryList.innerHTML = `<p class="empty-copy tournament-directory-empty">${escapeHtml(error.message || t('friendlyProblem'))}</p>`;
    window.setTimeout(() => {
      if (document.visibilityState === 'visible' && state.esportsView === 'tournaments') loadTournamentDirectory({ silent: true }).catch(() => null);
    }, 2500);
  } finally {
...
function queueBrowserCacheKey(region = state.queueRegion, mode = state.queueMode) {
  return `peakhalla-queue-v4:${region}:${mode}`;
}
...
function scheduleQueuePoll(data = state.queueData) {
  ...
  state.queueTimer = window.setTimeout(() => {
    ...
    loadLiveQueue({ silent: true, refresh, wait: true }).catch(() => null);
  }, delay);
}
...
async function loadLiveQueue(options = {}) {
  ...
  const activeRequestAge = Date.now() - Number(state.queueRequestStartedAt || 0);
  const replaceStuckRequest = Boolean(options.refresh || options.wait) && activeRequestAge > 8_000;
  if (state.queuePromise && state.queueRequestKey === requestKey && !replaceStuckRequest) return state.queuePromise;
  window.clearTimeout(state.queueTimer);
  if (state.queuePromise && (state.queueRequestKey !== requestKey || replaceStuckRequest)) state.queueController?.abort();

  const requestId = ++state.queueRequestId;
  const controller = new AbortController();
  state.queueRequestStartedAt = Date.now();
  const timeoutId = window.setTimeout(() => controller.abort(), options.wait || options.refresh ? 32_000 : 14_000);
...
function setLiveQueueMode(mode, options = {}) {
  ...
  if (!options.skipLoad) {
    ...
    const cached = readQueueBrowserCache(state.queueRegion, state.queueMode);
    if (cached) renderLiveQueue({ ...cached, status: cached.status || 'stale', __fromBrowserCache: true }, { schedule: false });
    loadLiveQueue({ silent: Boolean(cached), refresh: true, wait: true }).catch(() => null);
  }
}
...
els.queueRefresh?.addEventListener('click', () => loadLiveQueue({ refresh: true, wait: true }));
...
window.addEventListener('pageshow', (event) => {
  ...
  if (isLiveQueuePage) {
    startQueueHeartbeat();
    window.setTimeout(() => loadLiveQueue({ silent: true, refresh: queueScanIsDue(state.queueData), wait: true }).catch(() => null), 120);
  }
...
window.addEventListener('visibilitychange', () => {
  ...
  if (playerId && (!Number.isFinite(fetchedAt) || Date.now() - fetchedAt > 60_000)) {
    refreshPlayerRankedLive(playerId).catch(() => null);
    if (!state.playerLastVerifiedAt || Date.now() - state.playerLastVerifiedAt > 10 * 60_000) schedulePlayerLiveRefresh(playerId, 800);
  }
});
